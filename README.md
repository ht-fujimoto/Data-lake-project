...
このプロジェクトは、日本政府統計ポータル（e-Stat）から**全データセット（推定230,486件）**を取得し、Apache Iceberg形式でAWS S3に格納する包括的なデータレイクを構築します。データはAWS Athenaを通じてSQLクエリ可能で、効率的な分析とレポート生成をサポートします。

### 🎯 プロジェクトの目標

**E-stat全データをバッチ処理のみでデータレイクに格納する**

- MCPサーバー不要の完全自動化
- 段階的な実行で230,486件のデータセットを取得
- コスト最適化で年間$3,000-12,000
- 10-15日で全データセット取得完了（並列実行時）

### 主な特徴

- **全データセット対応**: E-stat全データ（推定230,486件）をバッチ処理で取得
- **11のドメインカテゴリ**: 人口、経済、労働、教育、保健・医療、農林水産、建設・住宅、運輸・通信、商業・サービス、社会保障、汎用
- **完全自動化**: MCPサーバー不要のバッチ処理システム
- **段階的実行**: 優先度に応じた段階的なデータ取得
...
```bash
# ステップ1: カタログ作成（E-stat全データセット一覧を取得）
python3 run_complete_batch_ingestion.py --phase catalog

# ステップ2: 優先度高データセット取得（100件）
python3 run_complete_batch_ingestion.py \
  --phase priority-high \
  --max-datasets 100

# ステップ3: 重要度高データセット取得（1,000件）
python3 run_complete_batch_ingestion.py \
  --phase important \
  --max-datasets 1000

# ステップ4: 全データセット取得（230,486件）
python3 run_complete_batch_ingestion.py \
  --phase all \
  --max-datasets 10000
```
...
| フェーズ | 対象 | 件数 | 実行時間 | 年間コスト |
|---------|------|------|---------|-----------|
| 0: カタログ | 全データセット一覧 | 230,486件 | 10-30分 | - |
| 1: 優先度高 | 優先順位9-10 | 1,000件 | 10時間 | $200 |
| 2: 重要度高 | 重要度「高」 | 86,964件 | 3-4日 | $4,580 |
| 3: 全データ | 全データセット | 230,486件 | 10-15日 | $8,928 |

詳細は [COMPLETE_BATCH_INGESTION_PLAN.md](COMPLETE_BATCH_INGESTION_PLAN.md) を参照してください。
