...
print("=" * 80)
print("Educationドメインデータ検証レポート")
print("=" * 80)

# 1. 元データの統計
print("\n【1. 元データ（E-stat API）】")
s3_key = 'raw/0003061540/0003061540_20260121_174606.json'
response = s3_client.get_object(Bucket=s3_bucket, Key=s3_key)
source_data = json.loads(response['Body'].read().decode('utf-8'))
print(f"  データセットID: 0003061540")
print(f"  タイトル: 学校基本調査 - 総括")
print(f"  総レコード数: {len(source_data)}")
print(f"  サンプル（最初の3件）:")
for i, record in enumerate(source_data[:3], 1):
    print(f"    {i}. 年: {record.get('@time', 'N/A')}, カテゴリ: {record.get('@cat01', 'N/A')}, 値: {record.get('$', 'N/A')}")

# 2. Icebergテーブルの統計
print("\n【2. Icebergテーブル（education_data）】")

# レコード数
count_query = "SELECT COUNT(*) as total FROM education_data WHERE dataset_id = '0003061540'"
...
# サンプルデータ
sample_query = """
SELECT dataset_id, year, region_code, school_type, category, value, unit
FROM education_data 
WHERE dataset_id = '0003061540'
ORDER BY year, school_type
LIMIT 5
"""
...
for i, row in enumerate(result['ResultSet']['Rows'][1:], 1):
    ...
    school = data[3]['VarCharValue'] if data[3] else 'NULL'
    value = data[5]['VarCharValue'] if data[5] else 'NULL'
    unit = data[6]['VarCharValue'] if data[6] else 'NULL'
    print(f"    {i}. 年: {year}, 学校種別: {school}, 値: {value}, 単位: {unit}")
...
# 学校種別の種類
school_query = """
SELECT COUNT(DISTINCT school_type) as school_count
FROM education_data 
WHERE dataset_id = '0003061540'
"""
result = execute_query(school_query)
school_count = result['ResultSet']['Rows'][1]['Data'][0]['VarCharValue']
print(f"  学校種別数: {school_count}")
...
# 7. S3データ格納場所
print("\n【7. S3データ格納場所】")
print(f"  生データ (JSON): s3://{s3_bucket}/raw/0003061540/")
print(f"  Parquet: s3://{s3_bucket}/parquet/education/0003061540.parquet")
print(f"  Iceberg: s3://{s3_bucket}/iceberg-tables/education/")

print("\n" + "=" * 80)
print("✅ 検証完了: Educationドメインデータは正常にロードされています")
print("=" * 80)
